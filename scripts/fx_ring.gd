class_name FxRing
extends Node2D
## Throwaway expanding ring. Impact feedback, dash trails, counter pops.
...
static func pop(parent: Node, at: Vector2, r0: float, r1: float, c: Color, life: float = 0.25) -> FxRing:
	var f := FxRing.new()
